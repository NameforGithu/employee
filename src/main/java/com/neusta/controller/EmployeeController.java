...
public class EmployeeController {
    ...
    ResponseEntity<Map<String,String>> createEmployee (@RequestBody @Valid EmployeeDto employeeDto){
        Map<String,String> outputInfo = new HashMap<>();
        Employee employee = employeeService.createEmployee(employeeDto);
        log.info("Employee created with id " + employee);
        outputInfo.put("message", "Employee created with id " + employee.getId());
    ...
    ResponseEntity<Map<String,String>> addProgrammingLanguageToEmployee (@PathVariable(value = "employee_id") long employee_id,
                                                                         ...
                                                                         @Param("amountOfExperience") int amountOfExperience ) {
        Map<String,String> outputInfo = new HashMap<>();
        employeeService.addCapabilityAsProgrammingLanguage(employee_id, programming_language, amountOfExperience);
    ...
    @PostMapping("/addFrameworkToEmployee/{employee_id}")
    ResponseEntity<Map<String, String>> addFrameworkToEmployee (@PathVariable(value = "employee_id") long employee_id,
                                                                @Param("framework") String framework,
                                                                @Param("amountOfExperience") int amountOfExperience) {
        Map<String,String> outputInfo = new HashMap<>();
        employeeService.addCapabilityAsFramework(employee_id, framework, amountOfExperience);
        log.info("Programming language added to employee with id: " + employee_id);
        outputInfo.put("message", "Programming language added to employee with id: " + employee_id);
        return new ResponseEntity<>(outputInfo, HttpStatus.OK);
    }
    ...
    @GetMapping("/filterEmployeeByCapability")
    ResponseEntity <List<Employee>> filterEmployeeByCapability (@RequestBody CapabilityRequest capabilityRequest){
        List<Employee> employees = employeeService.findEmployeesByCapability(capabilityRequest);
